...
try:
    print("Calculado a Variabilidade dos Dados")
    # Variância
    ...
    # Nesse caso, a variância elevada indica alta dispersão
    ...
    # É a normalização da Variância. Por isso é mais facil a interpretação
    # Apresentar o quanto os dados estão afatstados da média, tanto para mais, quanto para menos 
    desvio_padrao = np.std(variancia)
    ...
    print(f"A diferença entre Média e Variância: {distancia_var_media} %")    
    ...
    print(f"Erro ao calcular a disprsão {e}")
...
try:
    ...
    plt.axvline(media_roubo_veiculo, color="red", linewidth=1)
    plt.axvline(mediana_roubo_veiculo, color="black", linewidth=1)
    ...
    plt.text(0.5,0.8, f"Curtose: {assimetria}", fontsize=10)
    plt.text(0.5,0.7, f"Variância: {assimetria}", fontsize=10)
